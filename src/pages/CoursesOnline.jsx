...
const ACCENT = "#CDDC2D";       // لمسات تمييز خفيفة
const INK = "#414042";          // نصوص
...
export default function CoursesOnline() {
    ...
    // بحث فوري مثل صفحة الكتب/المقالات
    ...
    // Debounce للبحث أثناء الكتابة + تحديث الـ URL
    useEffect(() => {
        const id = setTimeout(() => {
            const clean = q.trim();
            setDebouncedQ(clean);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                setErr("");
                const { data } = await api.get("/api/courses/onsite/", {
                    params: {
                        page,
                        page_size: pageSize,
                ...
                if (!mounted) return;
                setErr("تعذّر تحميل الدورات الحضورية الآن.");
            } finally {
    ...
    const onPageChange = (p) => {
        setPage(p);
        applySearchParams({ page: p, q: debouncedQ, featured: featuredParam === "1" });
        ...
        <div className="min-h-screen bg-slate-50/60">
            ...
            <section className="relative overflow-hidden">
                ...
                <div className="container mx-auto px-6 pt-16 sm:pt-20 pb-8 sm:pb-10 relative">
                    ...
                    >
                        تعلّم وجهًا لوجه مع الدكتور محمد
                    </motion.h1>
                    ...
                    <motion.p
                        ...
                        style={{ color: INK }}
                    >
                        شارك في الدورات الحضورية التي تتيح لك التفاعل المباشر مع المدرّب وباقي المتدرّبين،
                        واستفد من تجربة تعليمية عملية في بيئة مهنية.
                    </motion.p>
